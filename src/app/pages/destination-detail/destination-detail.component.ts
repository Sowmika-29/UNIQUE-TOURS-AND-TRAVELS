        ...
        <div class="absolute bottom-0 left-0 w-full p-8 md:p-16 z-20">
          <div class="max-w-7xl mx-auto">
            <span class="inline-block px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider mb-4"
                  [ngClass]="destination.type === 'Domestic' ? 'bg-[#a4dded] text-slate-800' : 'bg-amber-500/90 text-white'">
            ...
            <!-- Social Share Bar -->
            <div class="mt-8 flex gap-3 animate-heading" style="animation-delay: 0.3s">
              <button (click)="shareWhatsApp()" class="bg-[#25D366] text-white p-3 rounded-full hover:scale-110 transition-transform shadow-lg" title="Share on WhatsApp">
              ...
              <button (click)="shareFacebook()" class="bg-[#1877F2] text-white p-3 rounded-full hover:scale-110 transition-transform shadow-lg" title="Share on Facebook">
                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385h-3.047v-3.47h3.047v-2.642c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953h-1.514c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385c5.738-.9 10.126-5.864 10.126-11.854z"/></svg>
              </button>
              <button (click)="copyLink()" class="bg-slate-700 text-white p-3 rounded-full hover:scale-110 transition-transform shadow-lg" title="Copy Link">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div class="lg:col-span-2">
            ...
            <!-- PLACES -->
            <div class="mb-16">
               <h2 class="text-3xl font-bold text-slate-800 mb-8 border-b-4 border-[#a4dded] inline-block pb-2">Places to Visit in {{destination.name}}</h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  <a *ngFor="let place of destination.places; let i = index"
                    ...
                    <!-- View Details Badge -->
                    <div class="p-3 bg-[#f0f9fb] flex items-center justify-between group-hover:bg-[#a4dded] transition-colors">
                      <span class="text-[10px] font-black uppercase tracking-widest text-[#5dbcd2] group-hover:text-slate-900">View Highlights</span>
                      <svg class="w-4 h-4 text-[#a4dded] group-hover:text-slate-900" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M9 5l7 7-7 7"></path></svg>
                    </div>
                  </a>
                </div>
            </div>
            ...
            <div class="flex border-b border-slate-300 mb-8 space-x-10 overflow-x-auto">
              <button
                ...
                [ngClass]="activeTab === 'includes' ? 'border-[#a4dded] text-[#4ea8bd]' : 'border-transparent text-slate-500 hover:text-slate-800'"
                class="pb-4 text-xl font-bold border-b-4 transition-colors whitespace-nowrap">
                Includes / Excludes
              </button>
              <button
                (click)="activeTab = 'policies'"
                [ngClass]="activeTab === 'policies' ? 'border-[#a4dded] text-[#4ea8bd]' : 'border-transparent text-slate-500 hover:text-slate-800'"
                class="pb-4 text-xl font-bold border-b-4 transition-colors whitespace-nowrap">
                Policies
              </button>
            </div>
            ...
            <div *ngIf="activeTab === 'includes'" class="grid grid-cols-1 md:grid-cols-2 gap-8 bg-white p-8 md:p-10 rounded-2xl shadow-sm border border-slate-100 animate-heading">
              <div>
                <h3 class="text-2xl font-bold text-[#4ea8bd] mb-8">What's Included</h3>
                <ul class="space-y-4">
                  <li *ngFor="let item of includes" class="flex items-start text-slate-700 font-medium">
                     <svg class="w-6 h-6 text-[#a4dded] mr-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
                     <span>{{item}}</span>
                  </li>
                </ul>
              </div>
          ...
          <div class="lg:col-span-1">
            <div class="sticky top-32 bg-white rounded-3xl p-8 shadow-[0_20px_50px_rgba(164,221,237,0.15)] border-2 border-[#a4dded] flex flex-col items-center text-center mt-12 lg:mt-0">

               <div class="w-20 h-20 bg-[#f0f9fb] rounded-full flex items-center justify-center text-[#a4dded] mb-6 shadow-inner border border-[#a4dded]/20">
               ...
               <a [href]="'https://wa.me/' + APP_CONFIG.whatsapp + '?text=Hello, I’m interested in ' + destination.name + ' trip. Please share package details.'" target="_blank"
                  class="w-full bg-[#a4dded] text-slate-900 py-4 rounded-xl font-bold text-xl hover:bg-[#7fd1e8] transition-all"
>
                  Enquire Now
               </a>
    ...
    <div *ngIf="!isLoading && !destination" class="min-h-screen flex items-center justify-center bg-slate-50 pt-20">
      <div class="text-center animate-heading">
        ...
        <a routerLink="/explore" class="inline-block bg-[#a4dded] text-slate-900 px-8 py-3 rounded-full font-bold hover:bg-[#7fd1e8] transition-all shadow-lg">
...
export class DestinationDetailComponent implements OnInit {
  ...
  shareWhatsApp() {
    ...
    const text = `Check out this amazing tour package for ${this.destination?.name}: `;
